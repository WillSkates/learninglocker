...
{
	if (typeof getOptions.Opt == 'undefined') {
		getOptions.Opt = loadCredentialsFromEnvironment();
	}

	return getOptions.Opt;
}

export const createClient = () => {
	if (typeof createClient.redisClient == 'undefined') {
		const creds = loadCredentialsFromEnvironment();

		try {
			createClient.redisClient = new Redis(creds);
		} catch(e) {
			logger.error('Couldn\'t connect to Redis!', e);
		}
	}

	return createClient.redisClient;
};
